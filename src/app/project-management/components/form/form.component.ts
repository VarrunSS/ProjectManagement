...
export class FormComponent implements OnInit {
        ...
        if (true) {
            gapi.client.prediction.trainedmodels.predict({
                ...
                that._zone.run(() => {
                    ...
                    output = that.computeTopEmployees(response, output);
    ...
    private _dummyPredictAssignee(ind: number): any {
        ...
        if (true) {
            ...
            output = this.computeTopEmployees(res, output);
    ...
    public computeTopEmployees(res, output) {

        // sort based on score
        var topEmployees = res.result.outputMulti.sort(function (a, b) { return (a.score > b.score) ? -1 : ((b.score > a.score) ? 1 : 0); });
        ...
        output.employees.forEach((obj, ind) => {
            ...
            if (ind == 0) {
                obj.suitable = obj.suitable * 1.7;
            }
            else if (ind == 1) {
                obj.suitable = obj.suitable * 1.3;
            }
            else if (ind == 2) {
                obj.suitable = obj.suitable * 1;
            }
